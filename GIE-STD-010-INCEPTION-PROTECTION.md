...
$$R_{inc} = \sum_{k=1}^{m} \omega_k \cdot \text{Sim}(Output, Pattern_k)$$
Where:
...
$$\Delta_{res} = |Progress_{agent} - Progress_{human}|$$
...
$$\forall \text{Output} : R_{inc} > 0.15 \implies State = HITL\_REVIEW$$
